...
class DependencyModel(Module):
    ...
    def forward(self, inputs):
        ...
        # flattening the embeddings to (batch_size, 768)
        embedded_flattened = embedded.view(embedded.size(0), -1)  # (batch_size, 768)

        # passing through the hidden layer and applying ReLU

        hidden_output = relu(self.hidden(embedded_flattened))  # (batch_size, 128)

        # passing through the output layer to get logits
...
def train(model, loader):
    ...
    for idx, batch in enumerate(loader):
        ...
        # computing training accuracy for this epoch
        correct += sum(torch.argmax(predictions, dim=1) == targets)
        total += len(inputs)

        # running the backward pass to update parameters
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
